...
static
ast_t *parse_arg(ast_ctx_t *ctx, ast_t *node)////////PARSING DES
{
...
{
    while (ctx->act_tok.type == T_SEMICOLON)//TANT QUE C EST UN SEMICOLON ON regarde si c est pas un semicolon
        ctx->act_tok = get_next_token(ctx);
}
