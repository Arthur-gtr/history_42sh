...
int visit_pipes(ef_t *ef)
{
    ...
    ef->flags |= F_PIPE;//Active une node PIP
...
int visit_list(ef_t *ef, ast_t *node)
{
    ...
    for (size_t i = 0; i < node->list.sz; i++) {
        ef->flags &= ~F_PIPE;//Desactive le flag
        ef->act_node = node->list.nodes[i];
...
int visitor_launcher(ef_t *ef)
{
    ...
    ef->ctx->act_tok = get_next_token(ef->ctx);//ef->ctx contient tous le getline - la premiere commande
    ef->ctx->ast = parse_expression(ef->ctx);//JE DOIS DETERMINER SE QUE FAIS PARSE EXPRESSION
    if (ef->ctx->ast == NULL)
        return RETURN_FAILURE;
    if (ef->ctx->ast->type == N_LST)//?? COMMAND COMPLEXE
